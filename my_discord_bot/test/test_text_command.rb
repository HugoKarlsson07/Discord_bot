...
class TestTextCommand < Minitest::Test
  ...
  def test_text_command_responds_with_text_1
    command = TextCommand.new(
      name: "hej",
      description: "grejt user",
      text: "välkommen!"
    )
    mock_event = MockEvent.new
    ...
    assert_equal 1, mock_event.responses.length
    assert_equal "välkommen!", mock_event.responses.first
  end
  def test_text_command_responds_with_text
    command = TextCommand.new(
      name: "Anka",
    ...
    assert_equal 1, mock_event.responses.length
    assert_equal "Welcome Anka!", mock_event.responses.first
  end
end